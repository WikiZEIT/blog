...
if (!file_exists($htmlFile)) {
    ...
    echo 'Strona wyszukiwania nie została zbudowana.';
    exit;
}
...
if ($query !== '' && file_exists($dbPath)) {
    try {
        ...
        $resultsHtml = '<div class="search-error">Wyszukiwanie jest tymczasowo niedostępne.</div>';
    }
} elseif ($query !== '') {
    $resultsHtml = '<div class="search-error">Indeks wyszukiwania nie został znaleziony.</div>';
}
...
if ($query !== '') {
    ...
    umami_track('/search/?q=' . urlencode($query), 'Szukaj: ' . $query, $_SERVER['HTTP_REFERER'] ?? '');
}
...
function renderResults(array $results, string $query): string {
    ...
    if ($count === 0) {
        return '<div class="search-results">'
             . '<p class="search-summary">Brak wyników dla <strong>&bdquo;' . $escaped . '&rdquo;</strong>. Spróbuj innych słów kluczowych.</p>'
             . '</div>';
    }
    ...
    $html = '<div class="search-results">'
          . '<p class="search-summary">' . $count . ' ' . $noun
          . ' dla <strong>&bdquo;' . $escaped . '&rdquo;</strong></p>';
